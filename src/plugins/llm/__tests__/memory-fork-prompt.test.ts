...
import { buildMemoryForkSystemPrompt } from '../memory-fork'
import { NO_UPDATE_MAGIC } from '../memory'

describe('buildMemoryForkSystemPrompt', () => {
  it('substitutes the existing memory placeholder', () => {
    const out = buildMemoryForkSystemPrompt('user likes hot dry noodles', 3000, 3300)
    expect(out).toContain('user likes hot dry noodles')
    expect(out).not.toContain('{{EXISTING_MEMORY}}')
  })

  it('shows "(空)" when memory is empty', () => {
    const out = buildMemoryForkSystemPrompt('', 3000, 3300)
